    ...
    match = match.replace(consts.tmplMxEventReg, (m, name, double, single) => { //查找事件
        ...
        if (double || single) {
            ...
            if (left > -1 && right > -1 && right > left) {
                let content = m.substring(left, right + 1);
                let prefix = m.substring(0, left);
                let postfix = m.substring(right + 1);
                let parts = jsGeneric.splitMxEvent(content);
                let passed = [],
                    opened;
                for (let p of parts) {
                    if (p == '({') {
                        ...
                        passed.push(encodeParams(`{${p}}`, refTmplCommands, originalMatch, e, toSrc));
                    } else {
                        passed.push(p);
                    }
                }
                content = passed.join('');
                let start = prefix.indexOf('=');
                let c;
                do {
                    c = prefix.charAt(start);
                    start++;
                } while (c != '"' && c != '\'');
                let rest = prefix.substring(start) + content + postfix;
                //console.log(rest);
                return prefix.substring(0, start) + holder + magixSpliter + rest;
            } else {
                return m;
            }
        }
