...
export default Designer.extend({
    ...
    '@:{drag.mod.point}'(element, e, key, host) {
        ...
        let { x: startX } = Transform["@:{transform#get.rotated.point}"](pos, c, start - rotate);
        ...
        host['@:{drag.drop}'](e, (evt) => {
            ...
            let { x: newX } = Transform["@:{transform#get.rotated.point}"](mpos, c, current - rotate);
    ...
    '@:{get.ctrl.points}'({ mod1X, width, height }) {
        let x = width * mod1X;
        return [{
            x,
            y: height / 2,
