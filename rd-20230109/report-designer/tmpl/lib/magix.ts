...
loader:umd
enables:mxevent,richVframe,xml,service,state,wait,load,lang,spreadMxViewParams,shortMX,base,richVframeById
optionals:removeStyle,waitSelector,batchDOMEvent,catchHTMLError,router,routerHash,routerState,routerTip,routerTipLockUrl,routerForceState,richView,eventVframe,innerView,richVframeInvokeCancel,richVframeDescendants,richVframeChildren,recast,require,remold,rebuild,rewrite,customTags,checkAttr,checkReset,webc,xview,taskComplete,preloadViews,esmoduleCheck,lockSubWhenBusy,taskIdle,taskCancel,inspector
*/
...
s.d('magix5', () => {
    ...
    let GUID = (prefix = MX_PREFIX) => prefix + Counter++;
    ...
    //let CacheCounter = 0;
    function MxCache(max: number = 50, buffer: number = 20, me?: any) {
        me = this;
        me['@:{cache#list}'] = [];
        me['@:{cache#buffer.count}'] = buffer; //buffer先取整，如果为0则再默认5
        me['@:{cache#max.count}'] = me['@:{cache#buffer.count}'] + max;
    ...
    let EventDefaultOptions: EventInit = {
        bubbles: true,
        cancelable: true
    };
    ...
    let ToParams = (params, keo) => {
        let arr = [], v, p;
        for (p in params) {
            v = params[p] + Empty;
            if (v || Has(keo, p)) {
                arr.push(Encode(p) + '=' + Encode(v));
            }
        }
        return arr.join('&');
    };
    let ToUri = (path, params, keo) => {
        let p = ToParams(params, keo);
        if (p) {
    ...
    let Async_Require = <T>(...names): Promise<T[]> => {
    ...
    let Vframe_Unmount = (me, node?, isVframeId?) => {
        let vf, pId;
        ...
        if (vf) {
            ...
            if (Has(vf?.['@:{vframe.children}'], node)) { //childrenMap
    ...
    let Vframe_UnmountZone = (owner, root?, onlyInnerView?) => {
        ...
        for (p in owner['@:{vframe.children}']) {
            ...
            if (unmount) {
                Vframe_Unmount(owner, p, unmount);
    ...
    let Vframe_mountView = async (owner, viewPath, viewInitParams?) => {
        ...
        if (root && viewPath) {
            ...
            [TView] = await Async_Require<FunctionConstructor>(view);
    ...
    Assign(Vframe[Prototype], {
        mount(node, viewPath, viewInitParams) {
            ...
            //CallFunction(Vframe_mountView,[vf,viewPath,viewInitParams,]);
 ...
 */
    ...
    //从dom属性中提取事件的正则\x1e(\d+)?(\x1e)为向上查找同类型事件的跳级，编译工具提供的性能优化
    let Body_EvtInfoReg = /^([\w\-]+)\x1e(\d+)?(\x1e)?([^(]+)\(([\s\S]*?)\)$/;
    //绑定在body上的所有事件
    let Body_RootEvents = {};
    //需要搜索选择器的事件，通常view中通过选择器绑定的事件会出现在这个对象里
    let Body_SearchSelectorEvents = {};

    //事件options对象缓存，用于添加或移除事件
    let Body_RootEvents_Modifier = {};
    //事件标识，是否是capture、passive等
    ...
    */
    ...
    let Body_DOMEventProcessor = domEvent => {
        ...
        while (target &&
            ...
            if (target.nodeType == offset) {
                ...
                if (offset) {
                    vframe = Vframe_Vframes[target['@:{node#vframe.id}']];
                    if (vframe?.['@:{vframe.view.entity}']?.['@:{view.events.object}']?.[type]) {
                        arr.length = 0;
                    }
                }
            }
        ...
        for (target of arr) {
            ignore = (target['@:{node#ignore.events}'] ||= {});
            ignore[type] = 1;
        }
    };
    let Body_DOMEventBind = (type, searchSelector, remove, flags) => {
        let counter = Body_RootEvents[type] | 0;//该类型的事件出现了多少次
        let flag = (Body_RootEvents_Flags[type] ||= {});//该类型事件标识
        //根据添加或移除，进行标识的增减
        let offset = (remove ? -1 : 1),
            fn = remove ? RemoveEventListener : AddEventListener;
        //对全局标识进行增加或减少，便于改变事件的options
        ...
        } else if (mod != lastMod) {//事件标识不同时，移除重新添加
            ...
            ready = () => {
                if (view['@:{view.sign}']) {
                    ...
                    if (view['@:{view.async.count}'] > 1) {
                        ...
                        CallFunction(Updater_Digest, view);
                    ...
                    if (lastVDOM['@:{v#node.tag}'] == V_TEXT_NODE) {
                        ...
                        */
                        if (updateAttribute) {
                            ...
                            V_SetAttributes(realNode, newVDOM, ref, lastVDOM);
                        ...
                        if (newMxView && oldVf &&
                            ...
                            if (paramsChanged ||
                                ...
                                // };
                                //updateAttribute = 1;
                                if (ToTry(assign, [params, newHTML], view) !== false) {
        ...
        if (!oView[Spliter]) { //只处理一次
            ...
            for (p in prop) {
                ...
                if (matches) {
                    ...
                    mod = modifiers ? ToObject(modifiers) : Body_Capture_True_Passive_True_Modifier;
                    events = events.split(Comma);
                    for (item of events) {
                        node = View_Globals[selectorOrCallback];
                        mask = 0;
                        mask |= (mod.passive || mod.passive == Null ? Body_Passive_True_Flag : Body_Passive_False_Flag);
                        mask |= (mod.capture ? Body_Capture_True_Flag : Body_Capture_False_Flag);
                        if (isSelector) {
                            if (node) {
                                eventsList.push({
    ...
    (<any>Base).extend = function extend(props, statics) {
        let me = this;
        let ctor = props?.ctor;
        function X(...a) {
            let t = this;
