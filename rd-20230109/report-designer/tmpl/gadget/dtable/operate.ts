...
let queryCellStatus = (table) => {
    ...
    if (focusRow > -1 && focusCol > -1) {
        ...
        let notDataAndTotalRow = dest['@:{start.row.index}'] > totalRowIndex || dest['@:{end.row.index}'] < dataRowIndex;
        canInsertRowTop = notDataAndTotalRow;
        canSplitH = canSplitV;
        canInsertRowBottom = notDataAndTotalRow;
...
export default View.extend({
    ...
    '@:{add.row}<click>'(e) {
        ...
        if (focusRow != -1 &&
            ...
            TableProvider["@:{table.provider#insert.row.at}"](props, index, getDefaultProps(), Const['@:{const#to.unit}'](30 * scale));
    ...
    '@:{add.col}<click>'(e) {
        ...
        if (focusRow != -1 &&
            ...
            TableProvider["@:{table.provider#insert.col.at}"](props, index, getDefaultProps(), Const['@:{const#to.unit}'](150 * scale));
            TableProvider["@:{table.provider#add.ext.meta}"](props);
            dest = rows[focusRow];
            if (to != 'left') {
                //let ci = 0;
