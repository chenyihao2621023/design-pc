...
export default View.extend({
    ...
    assign({ props, defined, disabled }) {
        ...
        let state = queryCellStatus(props);
        this['@:{has.elements}'] = defined.sub;
        this.set({
            props,
            disabled,
    ...
    '@:{add.row}<click>'(e) {
        ...
        if (focusRow != -1 &&
            ...
            if (this['@:{has.elements}']) {
                ...
                cellProps = getDefaultProps();
            }
            TableProvider["@:{table.provider#insert.row.at}"](props, index, cellProps, Const['@:{const#to.unit}'](30 * scale));
    ...
    '@:{add.col}<click>'(e) {
        ...
        if (focusRow != -1 &&
            ...
            TableProvider["@:{table.provider#insert.col.at}"](props, index, cellProps, Const['@:{const#to.unit}'](150 * scale));
            TableProvider["@:{table.provider#add.ext.meta}"](props);
            dest = rows[focusRow];
            if (to != 'left') {
                //let ci = 0;
    ...
    '@:{split.cell}<click>'(e) {
        ...
        if (focusRow != -1 &&
            ...
            TableProvider["@:{table.provider#split.cell}"](props, h, cellProps, min, min);
            TableProvider["@:{table.provider#add.ext.meta}"](props);
            dispatch(this.root, 'change', {
                use: 'rows',
                pkey: 'rows',
